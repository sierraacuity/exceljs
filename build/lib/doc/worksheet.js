...
var _require = require('../utils/copy-style'),
  copyStyle = _require.copyStyle;
  ...
  _createClass(Worksheet, [{
    ...
    value: function addRows(value) {
      ...
      value.forEach(function (row) {
        rows.push(_this3.addRow(row, style));
      });
      return rows;
    }
  }, {
    ...
    value: function duplicateRow(rowNum, count) {
      ...
      var _loop2 = function _loop2() {
        var rDst = _this4._rows[rowNum + i];
    ...
    value: function spliceRows(start, count) {
      ...
      if (nExpand < 0) {
        ...
        var _loop3 = function _loop3() {
          rSrc = _this5._rows[i - 1];
          if (rSrc) {
            var rDst = _this5.getRow(i + nExpand);
            ...
            _this5._rows[i - 1] = undefined;
          } else {
            _this5._rows[i + nExpand - 1] = undefined;
          }
        };
          ...
          if (rSrc) {
            ...
            rSrc.eachCell({
              ...
              if (cell._value.constructor.name === 'MergeValue') {
                var cellToBeMerged = _this5.getRow(cell._row._number + nInserts).getCell(colNumber);
                var prevMaster = cell._value._master;
                var newMaster = _this5.getRow(prevMaster._row._number + nInserts).getCell(prevMaster._column._number);
    ...
    value: function protect(password, options) {
      ...
      return new Promise(function (resolve) {
        ...
        if (password) {
          _this6.sheetProtection.algorithmName = 'SHA-512';
          _this6.sheetProtection.saltValue = Encryptor.randomBytes(16).toString('base64');
          _this6.sheetProtection.spinCount = options && 'spinCount' in options ? options.spinCount : 100000; // allow user specified spinCount
          _this6.sheetProtection.hashValue = Encryptor.convertPasswordToHash(password, 'SHA512', _this6.sheetProtection.saltValue, _this6.sheetProtection.spinCount);
        }
        if (options) {
          _this6.sheetProtection = Object.assign(_this6.sheetProtection, options);
          if (!password && 'spinCount' in options) {
            delete _this6.sheetProtection.spinCount;
          }
        }
        resolve();
      });
    ...
    value: function _parseRows(model) {
      ...
      model.rows.forEach(function (rowModel) {
        var row = new Row(_this8, rowModel.number);
        _this8._rows[row.number - 1] = row;
        row.model = rowModel;
      });
    }
  }, {
    ...
    value: function _parseMergeCells(model) {
      ...
      _.each(model.mergeCells, function (merge) {
        ...
        _this9.mergeCellsWithoutStyle(merge);
      });
    }
  }]);
  return Worksheet;
